...
var controller = {
    ...
    setEventListeners: function() {
        ...
        $('.btn-controller').on('click', function() {
            ...
            $.ajax(
                {
                    'url':      Routing.generate('send_commands_route', {'command': controller.commands[command]}),
                    'method':   'POST',
                    'dataType': 'json',
                ...
                $.ajax(
                    {
                        ...
                        'complete': function(data) {
                            ...
                            for (var i in zulus) {
                                var zulu = zulus[i];
                                $room.append('<option value="' + zulu.room + '">' + zulu.room + '</option>');
                            }
...
var app = {
    ...
    applyFancyTransitions: function() {
        $('body').on('click', '.link', function() {
            ...
            history.pushState(null, "Remote-Steuerung", address);
    ...
    setEventListeners: function() {
        ...
        $body.on('submit', '#settings-form', function() {
            var tab = $('.nav.nav-tabs').children('.active').find('a').attr('href');
            switch (tab) {
                case '#Applikation':
                    $('#ldap-host').remove();
                    $('#ldap-username').remove();
                    $('#ldap-password').remove();
                    $('#ldap-basedn').remove();
                    $('#ldap-domain').remove();
                    $('#ldap-domain-short').remove();
                    $('#ldap-filter').remove();
                    break;
                case '#LDAP':
                    $('#WebUntis').remove();
    ...
    updateEnvironment: function() {
        app.environment.isFullscreen = (screen.availHeight || screen.height - 30) <= window.innerHeight;
